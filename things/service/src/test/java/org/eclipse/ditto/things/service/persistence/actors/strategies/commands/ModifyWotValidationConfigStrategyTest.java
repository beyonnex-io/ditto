...
public final class ModifyWotValidationConfigStrategyTest {
    ...
    public void setUp() {
        ...
        when(ddata.add(any(ImmutableWotValidationConfig.class))).thenReturn(CompletableFuture.completedFuture(null));
    ...
    public void createNewConfig() {
        ...
        final ImmutableWotValidationConfig validationConfig = ImmutableWotValidationConfig.of(
                configId,
                true,
                false,
        ...
        final ModifyWotValidationConfig command = ModifyWotValidationConfig.of(configId, validationConfig, headers);

        // Ensure ddata returns null for the entity using the correct key
        final Key<ORSet<ImmutableWotValidationConfig>> key = ORSetKey.create("WotValidationConfig");
        when(ddata.get(eq(key), any(Replicator.ReadConsistency.class))).thenReturn(CompletableFuture.completedFuture(Optional.empty()));

        // When
        final Result<WotValidationConfigEvent<?>> result = underTest.apply(context, null, 1L, command);
        ...
        verify(visitor).onMutation(eq(command), eventCaptor.capture(), responseCaptor.capture(), eq(true), eq(false), eq(null));

        final WotValidationConfigEvent<?> event = eventCaptor.getValue();
        assertThat(event).isInstanceOf(WotValidationConfigCreated.class);
        final WotValidationConfigCreated createdEvent = (WotValidationConfigCreated) event;
        assertThat(createdEvent.getEntityId().toString()).isEqualTo(configId.toString());
    ...
    public void modifyExistingConfig() {
        ...
        final ImmutableWotValidationConfig newConfig = ImmutableWotValidationConfig.of(
                ...
                existingConfig.getDynamicConfig(),
