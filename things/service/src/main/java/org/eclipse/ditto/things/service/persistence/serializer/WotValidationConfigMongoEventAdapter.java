 ...
 * <p>
 * This adapter handles the serialization and deserialization of WoT validation config events for persistence.
 * It uses the same base configuration as the ThingMongoEventAdapter but is specialized for WoT validation config events.
 * </p>
 * <p>
 * The adapter tags events with appropriate journal tags derived from the event headers and type.
 * This allows for efficient querying and filtering of events in the journal.
 * </p>
 * <p>
 * Note: Currently no custom event migration is needed as WoT validation config events have a simpler structure
 * compared to Thing events (which require policy handling). If future schema changes require migration,
 * override {@link #performToJournalMigration(Event, JsonObject)} to implement the migration logic.
 * </p>
 */
public final class WotValidationConfigMongoEventAdapter extends AbstractMongoEventAdapter<WotValidationConfigEvent<?>> {
    ...
    protected JsonObjectBuilder performToJournalMigration(final Event<?> event, final JsonObject jsonObject) {
        ...
        // If future schema changes require migration, implement the logic here
