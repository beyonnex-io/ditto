...
public class WotValidationConfigExistenceChecker {
    ...
    private static final long VALIDATION_COOLDOWN_MS = 2000; // 2 seconds cooldown between validations of same ID

    private final ActorSystem actorSystem;
    private final WotValidationConfigDData ddata;
    private final Executor executor;
    private final AtomicReference<Set<ImmutableWotValidationConfig>> localConfigs;
    ...
    private void initialize() {
        LOGGER.info("Initializing WotValidationConfigExistenceChecker...");
        ddata.getConfigs()
                .thenAccept(configs -> {
                    final Set<ImmutableWotValidationConfig> initialConfigs = configs.getElements();
                    localConfigs.set(initialConfigs);
                    isInitialized.set(true);
                    LOGGER.info("WotValidationConfigExistenceChecker initialized with {} configs", initialConfigs.size());
                })
                .exceptionally(error -> {
    ...
    private static class DDataChangeListener extends AbstractActor {
        ...
        public void preStart() {
            ...
            isInitialized.set(true);
            LOGGER.info("DDataChangeListener started and subscribed to changes");
        }
            ...
            return receiveBuilder()
                    .match(Replicator.Changed.class, changed -> {
                        ...
                        if (changed.key().equals(ORSetKey.create("WotValidationConfig"))) {
                            final Set<ImmutableWotValidationConfig> newConfigs =
                                    ((ORSet<ImmutableWotValidationConfig>) changed.dataValue()).getElements();

                            // Update local cache with new configs (which may be empty if DData was cleared)
    ...
    private void updateLocalConfigs(final Set<ImmutableWotValidationConfig> newConfigs) {
        localConfigs.set(newConfigs);
        LOGGER.debug("Updated local configs cache with {} configs", newConfigs.size());
    }

    /**
     * Checks if a WoT validation config exists using a provided set of current configs (from a DData event) if available, otherwise falls back to local cache.
     *
     * @param configId the ID of the config to check
     * @param currentConfigs (optional) a set of current configs (from a DData event) to use for the check; if null, local cache is used.
     * @return a CompletionStage that completes with true if the config exists, false otherwise.
     */
    public CompletionStage<Boolean> checkExistence(final WotValidationConfigId configId, @Nullable final Set<ImmutableWotValidationConfig> currentConfigs) {
        if (currentConfigs != null) {
            final boolean exists = !currentConfigs.isEmpty() &&
                    currentConfigs.stream().anyMatch(config -> config.getConfigId().equals(configId));
            LOGGER.info("Local check for config {}: {}", configId, exists ? "exists" : "does not exist");
            return CompletableFuture.completedFuture(exists);
        } else {
            return checkExistence(configId);
        }
    }
    ...
    private CompletionStage<Boolean> doCheckExistence(final WotValidationConfigId configId) {
        // Check local cache
        final Set<ImmutableWotValidationConfig> currentConfigs = localConfigs.get();
        if (currentConfigs.isEmpty()) {
            LOGGER.info("Local cache is empty for config {}, returning false", configId);
            return CompletableFuture.completedFuture(false);
        }

        final boolean exists = currentConfigs.stream()
                .anyMatch(config -> config.getConfigId().equals(configId));
        LOGGER.info("Local cache check for config {}: {}", configId, exists ? "exists" : "does not exist");
        return CompletableFuture.completedFuture(exists);
    }
}
