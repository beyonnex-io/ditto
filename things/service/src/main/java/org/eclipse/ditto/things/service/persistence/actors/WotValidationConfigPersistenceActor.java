...
public final class WotValidationConfigPersistenceActor
        extends AbstractPersistenceActor<Command<?>, ImmutableWotValidationConfig, WotValidationConfigId, WotValidationConfigId, WotValidationConfigEvent<?>> {
    ...
    private final EventStrategy<WotValidationConfigEvent<?>, ImmutableWotValidationConfig> eventStrategy;
    ...
    @Override
    protected EventStrategy<WotValidationConfigEvent<?>, ImmutableWotValidationConfig> getEventStrategy() {
        return eventStrategy;
    }

    @Override
    protected JsonSchemaVersion getEntitySchemaVersion(final ImmutableWotValidationConfig entity) {
    ...
    @Override
    protected void publishEvent(final ImmutableWotValidationConfig entity, final WotValidationConfigEvent<?> event) {
    ...
    @Override
    protected CommandStrategy<Command<?>, ImmutableWotValidationConfig, WotValidationConfigId, WotValidationConfigEvent<?>> getCreatedStrategy() {
        return commandStrategies;
    }

    @Override
    protected CommandStrategy<Command<?>, ImmutableWotValidationConfig, WotValidationConfigId, WotValidationConfigEvent<?>> getDeletedStrategy() {
        return commandStrategies;
    }
    ...
    protected void recoveryCompleted(final RecoveryCompleted event) {
        ...
        LOGGER.info("recoveryCompleted called for entityId: {}. Entity: {}", entityId, entity != null ? entity.toString() : null);
        if (entity != null) {
            LOGGER.info("Starting DData update for recovered entity: {}", entity.getConfigId());
            ddata.add(entity)
                .whenComplete((v, error) -> {
                    if (error != null) {
                        LOGGER.error("Failed to publish WoT validation config to DData: {}", error.getMessage(), error);
                    } else {
                        LOGGER.info("Successfully published WoT validation config to DData: {}", entity.getConfigId());
                    }
                });
        } else {
